...
const Shell: React.FC<ShellProps> = ({ children, onLogout, activeTab, onNavigate }) => {
  ...
  const getCustomerNav = (): NavGroup[] => [
    { groupName: "Ecosystem", items: [
      ...
      { label: 'Bantuan', path: 'help', icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M18.364 5.636l-3.536 3.536m0 5.656l3.536 3.536M5.636 5.636l3.536 3.536m0 5.656l-3.536 3.536M9.172 9.172a4 4 0 115.656 5.656 4 4 0 01-5.656-5.656z" strokeWidth={2}/></svg> },
    ...
    <div className="min-h-screen bg-[#020617] text-slate-300 flex flex-col md:flex-row">
      ...
      {isCustomer && (
        <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-slate-950/80 backdrop-blur-xl border-t border-slate-900 px-6 py-3 flex justify-between items-center shadow-[0_-10px_20px_rgba(0,0,0,0.4)] overflow-x-auto no-scrollbar">
           ...
           <button onClick={() => onNavigate('repairs')} className={`flex flex-col items-center gap-1 shrink-0 px-2 ${activeTab === 'repairs' ? 'text-indigo-500' : 'text-slate-600'}`}>
              <ICONS.Ticket className="w-6 h-6" />
              <span className="text-[8px] font-black uppercase tracking-tighter">Servis</span>
           </button>
           ...
           <button onClick={() => onNavigate('orders')} className={`flex flex-col items-center gap-1 shrink-0 px-2 ${activeTab === 'orders' ? 'text-indigo-500' : 'text-slate-600'}`}>
