...
const CustomerDashboard: React.FC<CustomerDashboardProps> = ({ activeTab }) => {
  ...
  const getTitle = () => {
    switch (currentView) {
      case 'overview': return 'Beranda Node';
      case 'smart': return 'Neural Intelligence Hub';
      case 'marketplace': return 'Pasar Sumatra (Mall)';
      case 'cart': return 'Keranjang Belanja';
      case 'checkout': return 'Checkout Aman';
      case 'repairs': return 'Pusat Layanan (Service)';
      case 'warranty': return 'Pusat Garansi';
      case 'reviews': return 'Ulasan & Rating';
      case 'help': return 'Bantuan & Keluhan';
      case 'promo': return 'Promo & Loyalty';
      case 'notifications': return 'Kotak Masuk';
      case 'new-service': return 'Ajukan Servis Unit';
      case 'orders': return 'Ledger Pesanan';
      case 'payments': return 'Riwayat Finansial';
      case 'profile': return 'Profil & Pengaturan';
      default: return 'Terminal Pelanggan';
    }
  };
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-2 text-sm font-medium">Ekosistem Regional: <span className="text-indigo-400 font-bold">Sumatra-North-01</span></p>
        </div>
        ...
        <div className="flex gap-3">
           ...
           >
             <ICONS.Plus className="w-5 h-5" /> Servis Cepat
           </button>
        </div>
      </div>
